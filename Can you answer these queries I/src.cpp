...
void calc(ll ind, ll l, ll r, ll x, ll y){
	if(l == r){
		...
		maksimum[ind] = std::max(std::max(kraj[x] + pocetok[y], maksimum[x]), maksimum[y]);
...
void build(ll ind, ll l, ll r){
	if(l == r){
		...
		suma[ind] = 0;
		pocetok[ind] = kraj[ind] = maksimum[ind] = -10000000000000000LL;
		return;
	}
...
int main(){
	...
	std::fill(pocetok, pocetok + maxseg, -10000000000000000LL);
	std::fill(kraj, kraj + maxseg, -10000000000000000LL);
	std::fill(maksimum, maksimum + maxseg, -10000000000000000LL);
	std::fill(suma, suma + maxseg, 0);
	...
	for(ll i = 0; i < q; i++){
		...
		v_a = {-10000000000000000LL, -10000000000000000LL, -10000000000000000LL, 0};
		query(0, 0, n - 1, a, b);
		str << v_a[2] << '\n';
	}
